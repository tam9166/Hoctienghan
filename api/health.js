...
module.exports = async function handler(req, res) {
  ...
  const status = database.status === 'unreachable' ? 'degraded' : 'ok';
  res.setHeader('Cache-Control', 'no-store, max-age=0');
  return res.status(status === 'ok' ? 200 : 503).json({ status, backend: 'ok', database, ai, checkedAt: new Date().toISOString(), version: 'p33-v1' });
};
