...
const state = {
  ...
  ,resourceFilters: { type: 'all', level: 'all', search: '' }, selectedResourceId: '', selectedVideoId: '', selectedNoteId: '', selectedBookmarkType: 'all', videoChapterTime: 0
...
function syncShell() {
  ...
  document.querySelectorAll('.nav-item').forEach((button) => {
    ...
    const activeView = state.currentView === 'lesson' ? 'lessons'
      ...
      : ['profile','edit-profile','weekly-insights','progress-reports','notes','bookmarks'].includes(state.currentView) ? 'profile'
...
function lessonsView() {
  ...
  const groups = [
    ...
    ['한','Luyện viết chữ','Hangul theo nét và mức độ','handwriting'], ['💬','Tiếng Hàn thực tế','XKLĐ, du học và công sở','practical-korean'], ['📚','Học liệu','Grammar, audio, worksheet và reference','resources'], ['🎬','Bài giảng Video','Video Academy theo course và lesson','videos'], ['📝','Ghi chú của tôi','Ghi chú gắn với nội dung đang học','notes'], ['★','Đã lưu','Bookmark lesson, word, strategy và resource','bookmarks']
  ];
...
window.KLEARN_APP = { storage, state, STORAGE_KEYS, render, setView, toast, escapeHtml, normalizeSearch, getUserProgress, saveUserProgress, getUserSrs, saveUserSrs, userScoped, saveUserScoped, LearnerProfileService, MasteryService, PracticeService, CloudSyncService, AITutorService, PronunciationProvider, getDisplayPronunciation };
