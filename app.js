...
const CloudSyncService = {
  ...
  mergeVocabularyOrganization(local = [], remote = []) {
    const left = Array.isArray(local) ? local[0] || {} : {}; const right = Array.isArray(remote) ? remote[0] || {} : {};
    const stamp = (item = {}) => new Date(item.updatedAt || item.deletedAt || item.createdAt || 0).getTime() || 0; const folders = new Map();
    ...
    return [{ schemaVersion: 1, folders: [...folders.values()].slice(0, 1000), updatedAt: latest.updatedAt || null }];
  },
