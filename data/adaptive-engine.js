...
(function () {
  ...
  const RoadmapService = {
    ...
    async generateWithAI(goal = GoalTrackingService.getGoal()) { const fallback = this.get(goal); if (!window.AICoachService?.request) return { text: 'AI chưa sẵn sàng; đây là lộ trình luật deterministic từ dữ liệu local.', phases: fallback, source: 'rules' }; try { const text = await window.AICoachService.request(`Hãy đề xuất lộ trình học tiếng Hàn theo tuần cho mục tiêu TOPIK ${goal.targetLevel}, ${goal.dailyMinutes} phút/ngày, deadline ${goal.deadline || 'chưa đặt'}. Dựa duy nhất trên learner context; không bịa điểm số. Trả lời tiếng Việt ngắn gọn, có ưu tiên kỹ năng và cách đo tiến bộ.`); return { text, phases: fallback, source: 'ai+rules' }; } catch (error) { return { text: `${error.message} Lộ trình local vẫn được giữ bên dưới.`, phases: fallback, source: 'rules' }; } }
  };

  function missionCard() { const mission = DailyMissionService.getToday(); if (!mission) return ''; const top = mission.priorities || []; return `<section class="card section adaptive-mission-home"><div class="section-heading"><div><p class="eyebrow">🤖 AI Adaptive Learning</p><h2 class="section-title">Nhiệm vụ hôm nay</h2></div><span class="level-pill">${mission.totalMinutes} phút</span></div><p class="subtle">${mission.reducedPlan ? 'Kế hoạch rút gọn để khởi động lại nhịp học. ' : ''}${escapeHtml(mission.habitHint || '')}</p><div class="mission-priority-list">${top.map((item, index) => `<button class="mission-priority ${mission.completedItems?.includes(item.id) ? 'done' : ''}" data-adaptive-start="${item.id}"><span class="mission-rank">${index + 1}</span><span><b>${escapeHtml(item.title)}</b><small>${item.score} điểm ưu tiên · ${escapeHtml(item.reason)}</small></span><em>${item.minutes}′</em></button>`).join('')}</div><div class="action-row"><button class="btn primary" data-view="adaptive-plan">${mission.completed ? 'Xem lại kế hoạch' : 'Bắt đầu học'}</button><button class="btn secondary" data-view="adaptive-plan">Xem lộ trình</button></div></section>`; }
  ...
  function adaptiveView() { const mission = DailyMissionService.getToday(); const goal = GoalTrackingService.getGoal(); const phases = RoadmapService.get(goal); const progress = GoalTrackingService.progress(goal); const days = GoalTrackingService.daysRemaining(goal); return `<section class="section page-heading"><p class="eyebrow">🤖 Adaptive Learning Engine</p><h1 class="headline">Kế hoạch học của bạn</h1><p class="subtle">Ưu tiên được tính từ điểm kỹ năng, lỗi lặp, SRS đến hạn, thời gian bỏ trống và mục tiêu TOPIK. Cùng một ngày sẽ giữ nguyên kế hoạch khi tải lại.</p></section><section class="card section adaptive-mission"><div class="section-heading"><div><p class="eyebrow">Daily Mission · ${mission.date}</p><h2 class="section-title">${mission.totalMinutes} phút có chủ đích</h2></div><span class="sync-status">${mission.completed ? '✓ Đã hoàn thành' : `${mission.completedItems?.length || 0}/${mission.priorities.length} mục`}</span></div><div class="mission-priority-list">${mission.priorities.map((item, index) => `<article class="mission-priority ${mission.completedItems?.includes(item.id) ? 'done' : ''}"><span class="mission-rank">${index + 1}</span><span><b>${escapeHtml(item.title)} · ${item.score} điểm</b><small>${escapeHtml(item.reason)}</small></span><em>${item.minutes}′</em><button class="btn secondary" data-adaptive-start="${item.id}">Mở</button></article>`).join('')}</div><p class="subtle">${escapeHtml(mission.habitHint || '')}</p><button class="btn primary full" data-adaptive-complete="mission">${mission.completed ? 'Đã đánh dấu hoàn thành' : 'Đánh dấu nhiệm vụ đã hoàn thành'}</button></section><section class="card section adaptive-goal-form"><div class="section-heading"><div><p class="eyebrow">Goal Tracking</p><h2 class="section-title">Mục tiêu & deadline</h2></div><span class="level-pill">${progress}%</span></div><form id="adaptiveGoalForm" class="coach-form"><label>Mục tiêu<select name="goalType"><option value="topik" ${goal.goalType === 'topik' ? 'selected' : ''}>Thi TOPIK</option><option value="communication" ${goal.goalType === 'communication' ? 'selected' : ''}>Giao tiếp</option><option value="study" ${goal.goalType === 'study' ? 'selected' : ''}>Du học</option><option value="work" ${goal.goalType === 'work' ? 'selected' : ''}>Làm việc / XKLĐ</option></select></label><label>TOPIK mục tiêu<select name="targetLevel">${[1,2,3,4,5,6].map((level) => `<option value="${level}" ${Number(goal.targetLevel) === level ? 'selected' : ''}>TOPIK ${level}</option>`).join('')}</select></label><label>Phút mỗi ngày<select name="dailyMinutes">${[15,20,30,45,60].map((value) => `<option value="${value}" ${Number(goal.dailyMinutes) === value ? 'selected' : ''}>${value} phút</option>`).join('')}</select></label><label>Deadline<input type="date" name="deadline" value="${escapeHtml(goal.deadline || '')}" min="${dateKey()}" /></label><button class="btn primary full" type="submit">Lưu mục tiêu & tạo roadmap</button></form><div class="roadmap-phase-list">${phases.map((phase) => `<article class="roadmap-phase ${phase.status}"><div><span class="phase-index">${phase.index}</span><b>${escapeHtml(phase.title)}</b><small>${phase.weeks} tuần · ${escapeHtml(phase.rationale)}</small></div><span>${phase.status === 'active' ? 'Đang học' : 'Sắp tới'}</span></article>`).join('')}</div><button class="btn secondary full" id="generateAdaptiveRoadmap">✨ Tạo lộ trình bằng AI</button><div id="adaptiveRoadmapOutput" class="coach-output hidden"></div>${days !== null ? `<p class="subtle">Deadline còn ${days} ngày. Hệ thống sẽ ưu tiên mục tiêu này trong daily mission.</p>` : ''}</section>`; }
  ...
  window.KLEARN_AFTER_RENDER = () => {
    previousAfterRender?.();
    if (state.currentView === 'home' && !document.querySelector('.adaptive-mission-home')) document.getElementById('app')?.insertAdjacentHTML('beforeend', missionCard());
