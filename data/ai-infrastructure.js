...
(() => {
  ...
  window.KLEARN_AI_INFRASTRUCTURE = window.KLEARN_AI_INFRASTRUCTURE || {
    version: 1,
    status: 'approved',
    promptVersions: { default: 'p68-v1', tutor: 'p68-tutor-v1', coach: 'p68-coach-v1', grammar: 'p68-grammar-v1', grammar_support: 'p68-grammar-v1', sentence_correction: 'p68-correction-v1', speaking: 'p68-speaking-v1', speaking_feedback: 'p68-speaking-v1', realtime_voice_feedback: 'p68-voice-feedback-v1', weekly_report: 'p68-weekly-v1', personalized_practice: 'p68-practice-v1', learning_recommendation: 'p68-recommendation-v1', vocabulary_classification: 'p82-vocabulary-classification-v1', document_ocr: 'p83-document-ocr-v1', writing: 'p68-writing-v1', study_advisor: 'p68-advisor-v1', content_explanation: 'p68-explain-v1', practice_creator: 'p68-practice-v1', conversation_partner: 'p68-conversation-v1', writing_review: 'p68-writing-v1', career_coach: 'p68-career-v1', culture_advisor: 'p68-culture-v1' },
    experiments: { promptTone: { enabled: true, variants: ['clear', 'supportive'], allocation: [50, 50] } },
    routing: { simple: ['translation', 'definition', 'flashcard', 'tutor', 'study_advisor', 'learning_recommendation', 'vocabulary_classification', 'weekly_report', 'content_explanation'], strong: ['document_ocr', 'grammar', 'grammar_support', 'coach', 'speaking', 'speaking_feedback', 'short_feedback', 'realtime_voice_feedback', 'writing', 'sentence_correction', 'conversation', 'planning', 'practice_creator', 'personalized_practice', 'conversation_partner', 'writing_review', 'career_coach', 'culture_advisor'] },
  ...
  const buildContext = (context = {}, task = 'tutor') => {
    ...
    const trimOrder = ['recommendations', 'knowledgeGraph', 'recentMistakes', 'errorNotebook', 'relevantMemory', 'recentScores', 'learningMemorySummary', 'goalSimulation']; const limit = Number(config.limits?.maxContextChars || 9000); while (JSON.stringify(result).length > limit && trimOrder.length) { const field = trimOrder.shift(); if (Array.isArray(result[field])) result[field] = result[field].slice(0, Math.max(1, Math.floor(result[field].length / 2))); else delete result[field]; }
