...
(() => {
  ...
  const request = async ({ task = 'tutor', input = '', messages = [], context = {}, language = 'vi' } = {}) => {
    ...
    try {
      ...
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-KLearn-AI-Consent': 'granted' }, body: JSON.stringify({ messages: cleanMessages, learnerContext, learningLanguage: ['vi', 'en', 'zh-CN', 'ko', 'ja', 'zh'].includes(language) ? language : 'vi', task: clean(task, 40), modelRoute: route, promptVersion, privacy: { aiEnabled: true } }) });
