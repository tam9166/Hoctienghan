...
(function buildDailyLearningExperience(global) {
  ...
  if (!app) return;
  const { state, STORAGE_KEYS, render, setView, toast, escapeHtml, getUserProgress, userScoped, saveUserScoped, PracticeService, LearnerProfileService, VocabularyService } = app;
  const DURATIONS = Object.freeze([5, 15, 30, 45, 60, 90]);
  ...
  const DailyPlanService = {
    ...
    build(minutes = readState().preferredMinutes) {
      const duration = safeMinutes(minutes); const adaptive = global.AdaptiveLearningEngine?.generateLearningSession?.({ availableMinutes: duration });
      if (adaptive?.tasks?.length) return { minutes: adaptive.minutes, estimatedDuration: adaptive.estimatedDuration, recovery: adaptive.inactivity?.mode !== 'normal', inactivity: adaptive.inactivity, reason: adaptive.reason, evidenceSignature: adaptive.evidenceSignature, weaknesses: adaptive.weaknesses || [], generatedBy: adaptive.generatedBy, tasks: adaptive.tasks.map((item) => ({ ...item, route: item.route || item.actionView || routeFor(item.type) })) };
      const recovery = this.isRecovery(); const foundation = state.currentUser?.learningTrack === 'foundation'; const dueCount = VocabularyService?.dueCards?.().length || 0;
      if (recovery) {
        const slots = splitMinutes(duration, 3);
        return { minutes: duration, recovery: true, tasks: [
          dueCount ? { id: 'recovery-srs', type: 'srs', route: 'review', minutes: slots[0], title: copy(`Ôn ${Math.min(10, dueCount)} từ đến hạn`, `Review ${Math.min(10, dueCount)} due words`, `复习 ${Math.min(10, dueCount)} 个到期单词`), reason: copy('Lấy lại nhịp nhớ mà không học dồn.', 'Restart recall without cramming.', '轻量恢复记忆，不突击。') } : { id: 'recovery-foundation', type: 'lesson', route: routeFor('lesson'), minutes: slots[0], title: foundation ? copy('Tiếp tục nền tảng Hangul', 'Continue Hangul foundations', '继续韩文基础') : copy('Củng cố bài gần nhất', 'Refresh the latest lesson', '复习最近课程'), reason: copy('Không tạo nhiệm vụ ôn khi chưa có từ đã học.', 'No review task is created before vocabulary is learned.', '尚未学习词汇时不创建复习任务。') },
      ...
      if (duration === 5) return foundation ? { minutes: 5, recovery: false, tasks: [{ id: 'daily-foundation', type: 'lesson', route: 'foundation', minutes: 5, title: copy('Tiếp tục Hangul', 'Continue Hangul', '继续学习韩文'), reason: copy('Một bước nền tảng ngắn, đúng với trình độ hiện tại.', 'One short foundation step for your current level.', '完成一个适合当前水平的基础步骤。') }] } : { minutes: 5, recovery: false, tasks: [{ id: 'daily-quick', type: 'quick', action: 'quick', route: 'quick-practice', minutes: 5, title: 'Quick Practice', reason: copy('5 từ vựng · 3 câu nghe · 1 câu ngữ pháp.', '5 vocabulary · 3 listening · 1 grammar item.', '5 个词汇 · 3 道听力 · 1 道语法。') }] };
      let specifications = duration === 15
        ? [['srs', 5], [foundation ? 'lesson' : 'grammar', 5], ['listening', 5]]
        : duration === 30
          ? [['srs', 8], ['repair', 7], [foundation ? 'lesson' : 'grammar', 7], ['listening', 8]]
          : duration === 45
            ? [['srs', 10], ['repair', 8], [foundation ? 'lesson' : 'grammar', 9], ['listening', 10], ['reading', 8]]
            : duration === 90
              ? [['srs', 20], ['repair', 12], ['lesson', 18], [foundation ? 'reading' : 'grammar', 14], ['listening', 16], ['speaking', 10]]
              : [['srs', 15], ['lesson', 15], [foundation ? 'reading' : 'grammar', 10], ['listening', 10], ['speaking', 10]];
      if (!dueCount) { const removedMinutes = specifications.filter(([type]) => type === 'srs').reduce((sum, [, value]) => sum + value, 0); specifications = specifications.filter(([type]) => type !== 'srs'); if (specifications.length) specifications[0][1] += removedMinutes; }
  ...
  const DailyLearningSessionService = {
    ...
    start(minutes) {
      ...
      session.source = 'daily-experience'; session.engine = plan.generatedBy || 'daily-learning-experience'; session.recovery = plan.recovery; session.reason = plan.reason || plan.tasks[0]?.reason || ''; session.evidenceSignature = plan.evidenceSignature || ''; session.currentTask = 0; session.tasks = plan.tasks.map((task) => ({ ...task, completed: false }));
    ...
    completeCurrent() { const session = this.active(); if (!session) return null; global.FocusSessionService.completeTask(session); const active = this.active(); if (!active) { saveState({ lastCompletedAt: now(), lastEvidenceSignature: session.evidenceSignature || '' }); global.AdaptiveLearningEngine?.results?.record?.(session, {}); global.DailyMissionService?.complete?.('mission'); global.KLEARN_APP?.CloudSyncService?.schedule?.('adaptive-session-completed'); } return active || this.latestToday(); },
  ...
  function learningSnapshotMarkup() {
    const goal = global.GoalTrackingService?.getGoal?.(); const profile = global.AdaptiveLearningProfileService?.snapshot?.(); const weaknesses = global.WeaknessDetectionService?.detect?.({ learningProfile: profile })?.slice(0, 3) || []; const progress = getUserProgress?.() || {}; const todayMinutes = Number(progress.daily?.studyMinutes || 0); const targetMinutes = Number(goal?.dailyMinutes || state.currentUser?.studyMinutesPerDay || 15);
    return `<section class="daily-assistant-summary"><div><small>${copy('Trình độ', 'Level', '等级')}</small><b>${escapeHtml(goal?.currentLevel || profile?.currentLevel || state.currentUser?.level || 'Beginner')}</b></div><div><small>${copy('Mục tiêu', 'Target', '目标')}</small><b>TOPIK ${Number(goal?.targetLevel || profile?.targetTopikLevel || 2)}</b></div><div><small>🔥 Streak</small><b>${Number(progress.stats?.streak || 0)} ${copy('ngày', 'days', '天')}</b></div><div><small>⏱ ${copy('Hôm nay', 'Today', '今天')}</small><b>${todayMinutes} / ${targetMinutes} ${copy('phút', 'min', '分钟')}</b></div></section><section class="daily-weakness-panel"><div class="daily-section-heading"><div><p class="eyebrow">🧠 ${copy('Bạn đang yếu gì?', 'What needs work?', '哪些需要提高？')}</p><h2>${weaknesses.length ? copy('Ưu tiên cải thiện', 'Improvement priorities', '改进优先项') : copy('Cần thêm dữ liệu', 'More evidence needed', '需要更多数据')}</h2></div><button class="text-link" data-view="adaptive-plan">${copy('Xem phân tích', 'View analysis', '查看分析')}</button></div>${weaknesses.map((item) => `<article><span class="${item.priority}">${item.score}%</span><div><b>${escapeHtml(item.label)}</b><small>${escapeHtml(item.reason)}</small></div><button data-daily-open="${escapeHtml(({ listening: 'listening-studio', reading: 'practice-hub', grammar: 'grammar-compare', vocabulary: 'vocabulary-hub' })[item.skill] || 'adaptive-plan')}">${copy('Luyện ngay', 'Practise now', '立即练习')}</button></article>`).join('') || `<p class="subtle">${copy('Hoàn thành quiz hoặc đề TOPIK để nhận phân tích có căn cứ.', 'Complete a quiz or TOPIK test for evidence-based analysis.', '完成测验或 TOPIK 模考以获得分析。')}</p>`}</section>`;
  }
  function homeView() {
  ...
  global.KLEARN_AFTER_RENDER = () => {
    ...
    if (state.currentView === 'home' && !document.querySelector('.daily-assistant-summary')) document.querySelector('.daily-home-header')?.insertAdjacentHTML('afterend', learningSnapshotMarkup());
    const startToday = document.querySelector('[data-start-daily]'); if (startToday && !DailyLearningSessionService.active()) startToday.textContent = copy('▶ BẮT ĐẦU HỌC HÔM NAY', '▶ START TODAY’S SESSION', '▶ 开始今日学习');
