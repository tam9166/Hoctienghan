...
(function () {
  ...
  function dnaCard() { const dna = LearningMemoryService.summary(); const graph = KnowledgeGraphService.weaknessAnalysis().slice(0, 3); const chips = (items, fallback) => items.length ? items.map((item) => `<span class="memory-chip">${escapeHtml(item.content || item)}</span>`).join('') : `<span class="subtle">${fallback}</span>`; return `<section class="card section learning-dna"><div class="section-heading"><div><p class="eyebrow">🧬 AI Learning DNA</p><h2 class="section-title">AI hiểu bạn</h2></div><span class="sync-status">${LearningMemoryService.all().length} memories</span></div><div class="dna-grid"><div><h3>✓ Điểm mạnh</h3><div class="memory-chip-list">${chips(dna.strengths, 'Đang thu thập dữ liệu')}</div></div><div><h3>⚠ Điểm yếu</h3><div class="memory-chip-list">${chips(dna.weaknesses, 'Chưa có tín hiệu rõ')}</div></div><div><h3>Phong cách học</h3><div class="memory-chip-list">${chips(dna.preferences, 'Chưa ghi nhận lựa chọn')}</div></div><div><h3>Chủ đề cần cải thiện</h3><div class="memory-chip-list">${graph.length ? graph.map((node) => `<span class="memory-chip">${escapeHtml(node.label)} · ${node.weaknessScore}</span>`).join('') : '<span class="subtle">Graph chưa phát hiện điểm yếu</span>'}</div></div></div><p class="subtle">Chỉ lưu tín hiệu học tập quan trọng theo tài khoản; không lưu password, token hoặc toàn bộ hội thoại.</p></section>`; }
  function coachMemoryCard() { const memories = MemoryRetrievalService.retrieve('grammar vocabulary speaking listening goal', { limit: 3 }); const graph = KnowledgeGraphService.weaknessAnalysis()[0]; return `<section class="card section memory-insight"><p class="eyebrow">🧠 Long-term Memory</p><h2 class="section-title">AI nhớ điều quan trọng</h2>${memories.length ? `<ul>${memories.map((item) => `<li>${escapeHtml(item.content)}</li>`).join('')}</ul>` : '<p class="subtle">AI sẽ ghi nhớ điểm yếu, mục tiêu và sở thích học khi có đủ tín hiệu.</p>'}${graph ? `<p class="subtle">Knowledge Graph đang ưu tiên <b>${escapeHtml(graph.label)}</b> và các kiến thức liên quan.</p>` : ''}</section>`; }
