...
(function buildMonetizationFoundation(global) {
  ...
  function businessView() { if (!runtime.content) return loading(); const subscription = CommercialSubscriptionService.status(); const license = OrganizationPackageService.licenseStatus(); return `${heading('profile', 'P39 · BUSINESS FOUNDATION', 'Trung tâm thương mại', 'Quản lý gói dịch vụ, ưu đãi, giới thiệu, giấy phép và hỗ trợ trong một luồng rõ ràng.')}<section class="biz-status section"><article><small>GÓI CÁ NHÂN</small><b>${subscription.tier === 'premium' ? 'Premium' : 'Free'}</b><span>${escapeHtml(subscription.status)}</span></article><article><small>GIẤY PHÉP TỔ CHỨC</small><b>${escapeHtml(license.plan)}</b><span>${escapeHtml(license.status)}</span></article><article><small>THANH TOÁN</small><b>Server xác nhận</b><span>Không lưu dữ liệu thẻ trên trình duyệt</span></article></section><section class="biz-launch-grid section"><button data-view="billing-center"><span>01</span><b>Gói & thanh toán</b><small>Free, Premium, coupon và referral</small></button><button data-view="organization-plans"><span>02</span><b>Doanh nghiệp & trường học</b><small>Gói nhóm, seat và yêu cầu tư vấn</small></button><button data-business-support><span>03</span><b>Hỗ trợ khách hàng</b><small>Subscription, hóa đơn và license</small></button>${role() === 'admin' ? '<button data-view="revenue-center"><span>04</span><b>Revenue Analytics</b><small>Chỉ số tổng hợp, không có PII</small></button><button data-view="crm-center"><span>05</span><b>CRM Foundation</b><small>Pipeline tối giản cho tổ chức</small></button>' : ''}</section><section class="biz-principles section"><b>Ranh giới an toàn</b><ul><li>Học cốt lõi luôn thuộc gói Free.</li><li>Client không thể tự cấp Premium hoặc reward.</li><li>Checkout dùng idempotency key; webhook là nguồn xác nhận.</li></ul></section>`; }
