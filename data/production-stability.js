...
(() => {
  ...
  const allowedQueueActions = new Set(['completed_lesson', 'vocabulary_updated', 'srs_updated', 'mastery_updated', 'practice_completed', 'topik_completed', 'listening_completed', 'speaking_completed', 'writing_completed', 'favorite_updated', 'error_notebook_updated', 'achievement_updated', 'session_updated']);
  const safeQueuePayload = (input = {}) => Object.fromEntries(Object.entries(object(input)).filter(([name]) => !/(password|token|secret|authorization|api.?key)/i.test(name)).filter(([name]) => !['id', 'mutationId', 'type', 'entityId', 'lessonId', 'wordId', 'quizId'].includes(name)).map(([name, value]) => [clean(name, 60), typeof value === 'string' ? clean(value, 300) : typeof value === 'number' || typeof value === 'boolean' || value === null ? value : undefined]).filter(([, value]) => value !== undefined));
  const BackgroundSyncQueueService = {
    timer: null,
    all() { return array(userScoped?.(queueKey)); },
    enqueue(input = {}) { const requestedType = clean(input.type, 40); const type = queueAliases[requestedType] || requestedType; if (!allowedQueueActions.has(type) || !uid() || uid() === 'anonymous') return null; const entityId = clean(input.entityId || input.lessonId || input.wordId || input.quizId, 100); if (!entityId) return null; const mutationId = clean(input.mutationId || input.id || `${type}:${uid()}:${entityId}:${Date.now().toString(36)}`, 180); const existing = this.all().find((entry) => entry.mutationId === mutationId || entry.id === mutationId); if (existing) return existing; const item = { id: mutationId, mutationId, type, entityId, domain: clean(input.domain || type.replace(/_(completed|updated)$/, ''), 80), action: clean(input.action || type, 60), payload: safeQueuePayload(input), status: 'pending', retryCount: 0, lastAttemptAt: null, lastError: '', queuedAt: now() }; saveUserScoped?.(queueKey, [item, ...this.all()].slice(0, 500), 500); this.schedule(); return item; },
    schedule() { if (global.navigator?.onLine === false || typeof global.setTimeout !== 'function') return false; global.clearTimeout?.(this.timer); this.timer = global.setTimeout(() => this.flush(), 1500); return true; },
    markFailed(items, error) { const attemptedAt = now(); const message = clean(error?.code || error?.message || 'SYNC_RETRY_REQUIRED', 120); const ids = new Set(items.map((item) => item.mutationId)); const next = this.all().map((item) => ids.has(item.mutationId) ? { ...item, status: 'pending', retryCount: Number(item.retryCount || 0) + 1, lastAttemptAt: attemptedAt, lastError: message } : item); saveUserScoped?.(queueKey, next, 500); return next; },
    async flush() { const items = this.all(); if (!items.length) return { flushed: 0, pending: 0, status: 'idle' }; if (global.navigator?.onLine === false) return { flushed: 0, pending: items.length, status: 'offline' }; if (!CloudSyncService?.isConfigured?.()) return { flushed: 0, pending: items.length, status: 'local' }; try { const synced = await CloudSyncService.flush?.('background-queue'); if (synced !== true) { this.markFailed(items, new Error('SYNC_NOT_CONFIRMED')); return { flushed: 0, pending: items.length, status: 'error' }; } const sent = new Set(items.map((item) => item.mutationId)); saveUserScoped?.(queueKey, this.all().filter((item) => !sent.has(item.mutationId)), 500); return { flushed: items.length, pending: this.pending(), status: 'synced' }; } catch (error) { this.markFailed(items, error); ProductionMonitoringService.captureError({ type: 'sync', module: 'background-queue', error }); return { flushed: 0, pending: this.pending(), status: 'error' }; } },
