...
(function vocabularyImmersionSystem(global) {
  ...
  const runtime = state.p79Vocabulary || (state.p79Vocabulary = { config: null, loading: null, topicId: 'greetings', wordId: '', practiceMode: 'listening', practiceResult: null, capture: null, meaningVisible: true });
  ...
  function collections() { return (global.VocabularyCollectionService?.all?.() || userScoped?.(STORAGE_KEYS.vocabularyCollections) || []).filter((item) => !item.deletedAt); }
  ...
  function listeningVocabularyMarkup(word, options = {}) {
    const showMeaning = options.showMeaning ?? runtime.meaningVisible !== false;
    return `<div class="p79-listening-vocabulary"><small>🇰🇷 TIẾNG HÀN</small><h1 lang="ko">${escapeHtml(word.korean)}</h1><div class="p79-audio-pair" aria-label="Nghe phát âm tiếng Hàn"><button data-p79-audio="${escapeHtml(word.korean)}" data-rate="1" aria-label="Nghe ${escapeHtml(word.korean)} ở tốc độ bình thường">🔊 Bình thường</button><button data-p79-audio="${escapeHtml(word.korean)}" data-rate="0.7" aria-label="Nghe ${escapeHtml(word.korean)} chậm">🐢 Nghe chậm</button></div>${word.pronunciation ? `<details class="p79-romanization"><summary>Phiên âm (tùy chọn)</summary><p>${escapeHtml(word.pronunciation)}</p></details>` : ''}<div class="p79-korean-example"><b lang="ko">${escapeHtml(word.example || '')}</b>${word.exampleMeaning ? `<span>${escapeHtml(word.exampleMeaning)}</span>` : ''}</div><div class="p79-vietnamese-meaning" ${showMeaning ? '' : 'hidden'}><small>🇻🇳 TIẾNG VIỆT</small><p>${escapeHtml(word.meaning)}</p></div><button class="p79-meaning-toggle" data-p79-toggle-meaning aria-expanded="${showMeaning}">${showMeaning ? 'Ẩn nghĩa tiếng Việt' : 'Hiện nghĩa tiếng Việt'}</button></div>`;
  }

  function libraryView() {
    if (!runtime.config) return loading(); const analytics = VocabularyAnalyticsService.summary();
    return `<div class="p79-shell">${header('Thư viện từ vựng theo chủ đề', 'Chủ đề → học đa kỹ năng → bằng chứng thành thạo thật.')}<section class="section p79-stats"><article><b>${analytics.totalWords}</b><span>Tổng từ</span></article><article><b>${analytics.activeWords}</b><span>Đang học</span></article><article><b>${analytics.masteredWords}</b><span>Thành thạo</span></article><article><b>${analytics.weakWords}</b><span>Từ yếu</span></article></section>${['TOPIK I','TOPIK II'].map((level) => `<section class="section"><div class="p79-section-title"><div><small>${level}</small><h2>${level === 'TOPIK I' ? 'Nền tảng đời sống' : 'Ngôn ngữ học thuật & xã hội'}</h2></div></div><div class="p79-topic-grid">${VocabularyTopicLibraryService.all(level).map((topic) => `<button class="p79-topic-card" data-p79-topic="${topic.id}"><span>${topic.icon}</span><div><small>${escapeHtml(topic.level)}</small><h3>${escapeHtml(topic.title)}</h3><p>${escapeHtml(topic.description)}</p><div class="p79-progress"><i style="width:${topic.completion}%"></i></div><b>${topic.completedVocabulary}/${topic.totalVocabulary} từ · ${topic.completion}%</b></div></button>`).join('')}</div></section>`).join('')}<section class="section p79-actions"><button class="btn secondary" data-view="my-vocabulary-p82">🗂 My Vocabulary</button><button class="btn secondary" data-view="vocabulary-offline-p79">⇩ Offline packs</button><button class="btn secondary" data-view="vocabulary-analytics-p79">📊 Tiến độ</button></section></div>`;
  }
  ...
  global.KLEARN_AFTER_RENDER = () => {
    ...
    global.document?.querySelectorAll?.('[data-p79-learn]')?.forEach((button) => { button.onclick = () => { const word = findWord(button.dataset.p79Learn); if (!word) return; runtime.wordId = word.id; runtime.topicId = word.topicId || runtime.topicId; runtime.practiceResult = null; runtime.meaningVisible = true; VocabularyMasteryBridgeService.activate(word.id); setView('vocabulary-learn-p79'); }; });
    global.document?.querySelectorAll?.('[data-p79-audio]')?.forEach((button) => { button.onclick = () => speakKorean?.(button.dataset.p79Audio, Number(button.dataset.rate) || 1); });
    global.document?.querySelector?.('[data-p79-toggle-meaning]')?.addEventListener('click', () => { runtime.meaningVisible = runtime.meaningVisible === false; render(); });
