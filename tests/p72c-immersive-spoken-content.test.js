...
(async () => {
  ...
  assert.match(app, /immersiveSpoken: 'klearn_immersive_spoken'/); assert.match(app, /mergeImmersiveSpoken/); assert.match(app, /STORAGE_KEYS\.immersiveSpoken/); assert.doesNotMatch(index, /data\/immersive-spoken-content\.js\?v=1/); assert.match(index, /app\.js\?v=77/); assert.match(worker, /klearn-v91/); assert.match(worker, /content\/immersive-spoken-content\.json/); assert.match(loader, /immersiveSpoken: \{ dependencies: \['voice'\], styles: \['immersive-spoken-content\.css\?v=1'\], scripts: \['data\/immersive-spoken-content\.js\?v=1'\] \}/); assert.match(loader, /spoken-exercises speaking-flashcards spoken-shadowing/); assert.match(css, /@media\(max-width:600px\)/);
