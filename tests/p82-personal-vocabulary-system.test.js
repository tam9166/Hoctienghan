...
(async () => {
  ...
  const provenanceImport = w.P82ImportService.importIntoDeck(imported.deck.id, { fileName:'scan.pdf', sourceType:'smart-document', sourceFiles:[{ name:'scan.pdf', type:'pdf', pages:2 }], fingerprint:'p83-source', rows:[{ korean:'도서관', meaning:'Thư viện', sourceType:'pdf', sourceFileName:'scan.pdf', sourcePage:2, sourceSection:'Page 2', sourceOrder:4, sourceFingerprint:'p83-source:p2', extractionMethod:'ai-ocr', extractionConfidence:.82, reviewStatus:'confirmed' }], invalidCount:0, reviewedCount:1, extractionMethod:'ai-ocr' }, 'skip'); assert.equal(provenanceImport.added, 1); const provenanceWord = w.P82DeckService.words(imported.deck.id).find((word) => word.korean === '도서관'); assert.equal(provenanceWord.sourcePage, 2); assert.equal(provenanceWord.extractionMethod, 'ai-ocr'); assert.equal(provenanceWord.reviewStatus, 'confirmed'); assert.equal(w.P82DeckService.get(imported.deck.id).imports[0].sourceType, 'smart-document');
